...
// AR: لو مو POST رجّعه | EN: guard non-POST | 
...
$name      = trim($_POST["name"] ?? "");       // AR: الاسم | EN: name | 
$id_number = trim($_POST["id_number"] ?? "");  // AR: الهوية | EN: id number | 
$email     = trim($_POST["email"] ?? "");      // AR: الإيميل | EN: email | 
$city      = trim($_POST["city"] ?? "");       // AR: المدينة | EN: city |
...
$res = sb_insert_registration([ // AR: صف واحد | EN: one row | 
...
if (!$res['ok']) { // AR: فشل | EN: failed | SA: فشل
  header("Location: register.php?e=save"); exit; // AR: خطأ حفظ | EN: save error | 
}
